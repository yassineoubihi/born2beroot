...
<p><img width="828" alt="script" src="https://github.com/yassineoubihi/born2beroot/blob/main/src/Screen%20Shot%202023-12-12%20at%201.16.57%20AM.png?raw=true"></p>
...
pcpu=$(grep "physical id" /proc/cpuinfo | sort | uniq | wc -l)  # Count physical CPUs
vcpu=$(grep "^processor" /proc/cpuinfo | wc -l)                 # Count virtual CPUs
fram=$(free -m | awk '$1 == "Mem:" {print $2}')                 # Total RAM in MB
uram=$(free -m | awk '$1 == "Mem:" {print $3}')                 # Used RAM in MB
...
cpul=$(awk '/^cpu / {usage=($5+$6)*100/($2+$3+$4+$5+$6)} END {printf "%.1f%%\n", usage}' /proc/stat)  # CPU load percentage
lb=$(who -b | awk '$1 == "system" {print $3 " " $4}')  # Last system boot time
lvmu=$(if [ $(lsblk | grep lvm | wc -l) -eq 0 ]; then echo no; else echo yes; fi)  # Check if LVM is in use
ctcp=$(cat /proc/net/sockstat{,6} | awk '$1 == "TCP:" {print $3}')  # Count established TCP connections
ulog=$(users | wc -w)  # Count the number of logged-in users
ip=$(hostname -I)  # Get the system's IP address
mac=$(ip link show | awk '$1 == "link/ether" {print $2}')  # Get the system's MAC address
cmds=$(journalctl _COMM=sudo | grep COMMAND | wc -l)  # Count the number of sudo commands executed

# Display system information using 'wall' command to broadcast on all terminals
